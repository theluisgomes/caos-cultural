...
export const WorkCard: React.FC<WorkCardProps> = ({ listing, onClick }) => (
  <div className="group cursor-pointer flex flex-col gap-4" onClick={onClick}>
    <div className="relative aspect-[4/5] overflow-hidden rounded-sm bg-zinc-900">
      ...
      <div className="absolute top-3 left-3 bg-amber-600 text-white text-[10px] font-bold uppercase tracking-widest px-2 py-1">Obra</div>
      <InteractionBar targetType="work" targetId={listing.id} className="absolute bottom-3 right-3" />
    </div>
    <div>
      ...
      <p className="text-zinc-300 text-sm mt-1">{listing.price}</p>
    </div>
  </div>
);
